...
void BinaryTree:: display( std::ostream& outfile ) const
{
   std::string prefix;
   if( tree_ == NULL )
   {
...
{
	if (!subtree) return 0;
	int left_height = height(subtree->left_);
	int right_height = height(subtree->right_);
...
short BinaryTree::leftmost( const BinaryNode * subtree )
{
	...
	if (subtree == NULL) return 0;
	for (; subtree; subtree = subtree->left_)
		if (!subtree->left_) break;
